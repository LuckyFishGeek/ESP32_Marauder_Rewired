...
def write_headers(out_dir, defines_tokens, tft_pins, touch_pins, tft_wh=None):
    ...
    lines = [
        "// Autogenerated by gen_tft_setup_json.py",
        "#define USER_SETUP_LOADED"  # force TFT_eSPI to use this config
    ]
    ...
    w = h = None
    if tft_wh and isinstance(tft_wh, tuple) and len(tft_wh) == 2:
        w, h = tft_wh
    else:
        # Infer from model tokens if not overridden
        model_tokens = " ".join(defines_tokens).upper()
        if "ILI9341" in model_tokens:
            w, h = 240, 320
        elif "ST7789" in model_tokens:
            # default if not provided; can be overridden by input
            w, h = 240, 320
    ...
    # --- Write files ---
    autogen.write_text("\n".join(lines) + "\n", encoding="utf-8")
    # Use local-include so the sketch finds the autogen header in the build dir
    select.write_text(
