...
void showMainMenu() {
  ...
  logln(F("=== Marauder (Rewired) ==="));
  logln(F("1) Wi‑Fi tools"));
  logln(F("2) Bluetooth tools"));
  logln(F("3) Storage"));
  logln(F("4) Settings"));
  logln(F("--------------------------"));
...
void handleCommand(const String& cmd) {
  ...
  if (cmd == "1") {
    logln(F("[Custom] Wi‑Fi tools selected"));
  } else if (cmd == "2") {
    logln(F("[Custom] Bluetooth tools selected"));
  } else if (cmd == "3") {
    logln(F("[Custom] Storage selected"));
  } else if (cmd == "4") {
    logln(F("[Custom] Settings selected"));
